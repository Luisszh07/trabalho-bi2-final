...
> Diferente das demais dimensões, a Dim_Calendario **não possui relacionamento físico** com a Fato_Municipio no modelo final. Como o dataset representa um único período (snapshot), o relacionamento 1:N não agrega valor analítico real e foi mantido como tabela de referência desconectada, evitando a necessidade de adicionar uma chave fixa artificial em todas as linhas da fato.
...
- Dim_Calendario → *(desconectada, sem relacionamento físico)*
